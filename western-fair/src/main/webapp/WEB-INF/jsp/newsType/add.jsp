	...
	<script src="${pageContext.request.contextPath}/layer/layer.js"></script>
        <script type="text/javascript">
            ...
            $("#insertBtn").click(function(){
            	var name = $("#name").val();
            	if(name == ""){
            		layer.msg("类型名称不能为空，请输入", {time:2000, icon:5, shift:6}, function(){  	   	   
	    	    	   });
            		//用户名和邮箱的验证暂时省略
            		return;
            	}
            	...
            	$.ajax({
            		type : "POST",
            		url	 : "${pageContext.request.contextPath}/newsType/insert",
            		data :{
            			"name" : $("#name").val()
            		},
            		beforeSend : function(){
      				...
      				success : function(result){
      	    		   ...
      	    		   if(result.success){
      	    			 layer.msg("类型保存成功", {time:2000, icon:6}, function(){ 
      	    				 window.location.href = "${pageContext.request.contextPath}/newsType/index";
	    	    	   	 });  
      	    			 
      	    		   }else{
      	    			 layer.msg("类型保存失败", {time:2000, icon:5, shift:6}, function(){  	   	   
  	    	    	   		});  
      	    		   }
      	    		}
            	});
